...
export default function ListaDeseos({ isMobile }) {
    ...
    try {
        const res = await fetch("https://backend-production-9f93.up.railway.app/wishlist/list", {
  ...
  try {
    const res = await fetch(`https://backend-production-9f93.up.railway.app/wishlist/delete/${bookId}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
