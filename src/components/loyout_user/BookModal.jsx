...
export default function BookModal({ book, onClose, onAddToWishlist, isBookSaved, onRemoveFromWishlist, usuario, handleGuestAction, libro_id }) {
    ...
    const fetchUserRating = useCallback(async (olKey) => {
        ...
        try {
            const userRatingRes = await fetch(`https://backend-production-9f93.up.railway.app/reviews/user-rating/${key}`, {
                headers: {
    ...
    const fetchReviewsAndComments = useCallback(async (olKey) => {
        ...
        try {
            ...
            const ratingsRes = await fetch(`https://backend-production-9f93.up.railway.app/reviews/ratings/${key}`);
            const ratingsData = await ratingsRes.json();
            setAverageRating(ratingsData.promedio || 0.0);
            setTotalVotes(ratingsData.total_votos || 0);

            const commentsRes = await fetch(`https://backend-production-9f93.up.railway.app/reviews/comments/${key}`);
  ...
  try {
    ...
    const handleEditComment = async (newText) => {
        ...
        try {
            ...
            const res = await fetch(`https://backend-production-9f93.up.railway.app/reviews/comment/${commentId}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
    ...
    const handlePhysicalBorrow = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch("https://backend-production-9f93.up.railway.app/prestamos-fisicos/mis-prestamos", {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
...
const handleDigitalBorrow = async () => {
    ...
    try {
        const res = await fetch("https://backend-production-9f93.up.railway.app/prestamos/digital", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
