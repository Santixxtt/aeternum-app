...
export default function BookModal({
    ...
    async (olKey) => {
      ...
      try {
        const userRatingRes = await fetch(
          `https://backend-production-9f93.up.railway.app/reviews/user-rating/${key}`,
          {
            headers: {
  ...
  const fetchReviewsAndComments = useCallback(async (olKey) => {
    ...
    try {
      const [ratingsRes, commentsRes] = await Promise.all([
        fetch(
          `https://backend-production-9f93.up.railway.app/reviews/ratings/${key}`
        ),
        fetch(
          `https://backend-production-9f93.up.railway.app/reviews/comments/${key}`
        ),
      ]);
    ...
    const fetchDescription = async () => {
      ...
      try {
        if (book.es_local && book.descripcion_local) {
          setDescription(
            book.descripcion_local || "No hay resumen disponible."
          );
          setLoading(false);
          return;
        }
      ...
      try {
        ...
        for (const edition of editionsData.entries || []) {
          if (edition.ocaid) {
            iaId = edition.ocaid;
            console.log(
              "✅ Edición con Internet Archive encontrada:",
              edition.key,
              "->",
              iaId
            );
          ...
          if (edition.source_records) {
            const iaRecord = edition.source_records.find((r) =>
              r.startsWith("ia:")
            );
            if (iaRecord) {
  ...
  const handleDownload = () => {
    ...
    if (downloadInfo.hasAnyFormat) {
      ...
      alert(
        "📚 Este libro no tiene descarga directa disponible. Te redirigiremos a Open Library para ver opciones de préstamo digital con nuestro aliado."
      );
  ...
  const handleSubmitRating = async (newRating) => {
    ...
    try {
      const res = await fetch(
        "https://backend-production-9f93.up.railway.app/reviews/rate",
        {
          method: "POST",
          headers: {
  ...
  const handleSubmitComment = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://backend-production-9f93.up.railway.app/reviews/comment",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleEditComment = async (newText) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://backend-production-9f93.up.railway.app/reviews/comment/${commentId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!res.ok) {
        const err = await res.json();
        alert(
          `Error al editar: ${err.detail || "Error al actualizar el comentario."}`
        );
      } else {
        // ✅ NUEVO: Actualizar estado local directamente
        setComments((prevComments) =>
          prevComments.map((c) =>
            c.id === commentId ? { ...c, texto: trimmedText } : c
          )
        );
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        alert(
          `Error al eliminar: ${err.detail || "Error al eliminar el comentario."}`
        );
      } else {
        // ✅ NUEVO: Actualizar estado local directamente
        setComments((prevComments) =>
  ...
  const renderDescription = () => {
    ...
    if (!description || description === "No hay resumen disponible.")
  ...
  const CommentItem = ({ comment }) => {
    ...
    return (
      <div key={comment.id} className="comment-item">
        <p>
          <strong>
            {comment.nombre_usuario ?? comment.nombre ?? "Usuario Desconocido"}
          </strong>
        </p>
        <p>{comment.texto}</p>
  ...
  const handlePhysicalBorrow = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        "https://backend-production-9f93.up.railway.app/prestamos-fisicos/mis-prestamos",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDigitalBorrow = async () => {
    ...
    try {
      const res = await fetch(
        "https://backend-production-9f93.up.railway.app/prestamos/digital",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          <div className="modal-info">
            ...
            <section className="rating-section">
              ...
              {loadingReviews ? (
                ...
                <>
                  <p>
                    Promedio: <strong>{averageRating.toFixed(1)} / 5</strong> (
                    {totalVotes} votos)
                  </p>
                  <div className="star-rating">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <span
                        ...
                        onClick={() =>
                          !isSubmittingRating && handleSubmitRating(star)
                        }
                        style={{
                    ...
                    <p>
                      Mi calificación:{" "}
                      {rating > 0 ? `${rating} estrellas` : "Sin calificar"}
                    </p>
                  </div>
                </>
              )}
            ...
            <div className="modal-comments">
              ...
              {token && isEditing !== null ? (
                <CommentsEdit
                  initialText={
                    comments.find((c) => c.id === isEditing)?.texto || ""
                  }
                  onSave={handleEditComment}
                  onCancel={cancelEditComment}
                  placeholder={`Editando comentario...`}
                  nombreUsuario={
                    comments.find((c) => c.id === isEditing)?.nombre_usuario
                  }
                />
              ) : token ? (
            ...
            <div className="modal-actions">
              ...
              <button className="btn-digital" onClick={handleDigitalBorrow}>
              ...
              <button
                ...
                title={
                  downloadInfo.hasAnyFormat
                    ? "Ver formatos disponibles"
                    : "Ver en Open Library"
                }
                className={
                  downloadInfo.hasAnyFormat ? "btn-download-available" : ""
                }
              >
                <i
                  className={
              ...
              <button
                ...
                className={
                  isBookSaved ? "btn-remove-wishlist" : "btn-add-wishlist"
                }
              >
