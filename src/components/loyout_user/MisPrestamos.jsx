...
export default function Prestamos({ isMobile }) {
    ...
    const cargarPrestamos = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://10.17.0.28:8000/prestamos-fisicos/mis-prestamos?_t=${timestamp}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const cancelarPrestamo = async (prestamoId) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`http://10.17.0.28:8000/prestamos-fisicos/cancelar/${prestamoId}`, {
                method: "PUT",
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const getImageUrl = (prestamo) => {
        ...
        if (prestamo.imagen_local) {
            return `http://10.17.0.28:8000/${prestamo.imagen_local}`;
        }
        return defaultImage;
    };
