...
export default function Prestamos({ isMobile }) {
    ...
    const cargarPrestamos = async () => {
        ...
        try {
            ...
            const res = await fetch(`https://backend-production-9f93.up.railway.app/prestamos-fisicos/mis-prestamos?_t=${timestamp}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const cancelarPrestamo = async (prestamoId) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`https://backend-production-9f93.up.railway.app/prestamos-fisicos/cancelar/${prestamoId}`, {
                method: "PUT",
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            if (data.status === "success") {
                // ✅ ACTUALIZACIÓN INMEDIATA: Cambiar estado local sin esperar recarga
                setPrestamos(prevPrestamos =>
                    prevPrestamos.map(p =>
                        p.id === prestamoId ? { ...p, estado: "cancelado" } : p
                    )
                );
    ...
    // ✅ Filtrar con useMemo para optimizar
    const prestamosFiltrados = useMemo(() => {
        if (filtroActivo === "todos") return prestamos;
        return prestamos.filter(p => p.estado === filtroActivo);
    ...
    const contadores = useMemo(() => {
        return {
            ...
            atrasado: prestamos.filter(p => p.estado === "atrasado").length,
            devuelto: prestamos.filter(p => p.estado === "devuelto").length,
            cancelado: prestamos.filter(p => p.estado === "cancelado").length,
        };
    }, [prestamos]);

    const getEstadoBadge = (estado) => {
        const badges = {
            pendiente: { className: "badge-pendiente", text: "🟡 Pendiente", icon: "bx-time" },
            activo: { className: "badge-activo", text: "🟢 Activo", icon: "bx-check-circle" },
            atrasado: { className: "badge-atrasado", text: "🔴 Atrasado", icon: "bx-error" },
            devuelto: { className: "badge-devuelto", text: "⚪ Devuelto", icon: "bx-archive" },
            cancelado: { className: "badge-cancelado", text: "⚫ Cancelado", icon: "bx-x-circle" }
        };
    ...
    const getImageUrl = (prestamo) => {
        if (prestamo.cover_id) {
            return `https://covers.openlibrary.org/b/id/${prestamo.cover_id}-M.jpg`;
        }
        if (prestamo.imagen_local) {
            return `https://backend-production-9f93.up.railway.app/${prestamo.imagen_local}`;
        }
        return defaultImage;
    };
                ...
                <div className="perfil-container">
                    <section className="prestamos-section">
                        <div className="prestamos-header">
                            <h2>
                                <i className="bx bx-book-reader"></i> <span className="text-black">Mis Préstamos</span>
                            </h2>
                            <button 
                        ...
                        <div className="filtros-container">
                            ...
                            >
                                ...
                                Todos
                                <span className="badge-count">{contadores.todos}</span>
                            </button>
                            ...
                            >
                                ...
                                Pendientes
                                <span className="badge-count">{contadores.pendiente}</span>
                            </button>
                            ...
                            >
                                ...
                                Activos
                                <span className="badge-count">{contadores.activo}</span>
                            </button>
                            ...
                            >
                                ...
                                Atrasados
                                <span className="badge-count">{contadores.atrasado}</span>
                            </button>
                            ...
                            >
                                ...
                                Devueltos
                                <span className="badge-count">{contadores.devuelto}</span>
                            </button>
                            ...
                            >
                                ...
                                Cancelados
                                <span className="badge-count">{contadores.cancelado}</span>
                            </button>
                        </div>
                        ...
                        {prestamosFiltrados.length === 0 ? (
                            ...
                            <div className="prestamos-grid">
                                {prestamosFiltrados.map((prestamo) => {
                                    ...
                                    const imageUrl = getImageUrl(prestamo);
                                    ...
                                    return (
                                        <div key={prestamo.id} className="prestamo-card">
                                            ...
                                            <div className="prestamo-actions">
                                                <span className={`badge-estado ${badge.className}`}>
