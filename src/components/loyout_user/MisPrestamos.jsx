  ...
  try {
    ...
    const res = await fetch(`http://10.17.0.26:8000/prestamos-fisicos/mis-prestamos?_t=${timestamp}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  try {
    const token = localStorage.getItem("token");
    const res = await fetch(`http://10.17.0.26:8000/prestamos-fisicos/cancelar/${prestamoId}`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${token}` }
    });
